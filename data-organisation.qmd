...
A clear and consistent folder structure and folder and file naming convention are important for making your data [findable](https://www.go-fair.org/fair-principles/). You should think about it beforehand in order to avoid inconsistencies or the need to rename large amounts of data.
...
::: {.callout-tip collapse="false" icon="true" title="Task 1.1: (\~ 5 minutes)"}
Look at the folder structure (but not at the details of the folder or file names yes, which will be the next task).
...
::: {.callout-note collapse="true" icon="true" title="Further hints on folder structure"}
...
Dataset contains 6 files, whithout folder structure. However, 2 of them are of type 'tar.gz', which contain compressed ASCII files. Content is described in the README file.
...
- Yes, grouped into data, code (scripts) etc
- Described in README file, but not completely.
- Only up to 8 files, one folder level
:::
...
::: {.callout-tip collapse="false" icon="true" title="Task 1.2: (\~ 10 minutes)"}
...
::: {.callout-warning collapse="false" icon="true" title="No 'hello.txt' and 'Hello.txt' in same folder."}
...
::: {.callout-note collapse="true" icon="true" title="Excursion: Ordering and timestamps"}
...
::: {.callout-note collapse="true" icon="true" title="Further good practice for file naming"}

- Include relevant information in the file name. However, don't misuse a file name as a way to store all your metadata.
- Avoid overly long names (a maximum of 32 characters is suggested). Mind also note above about the full path length.
- Avoid to moving or renaming folders or files. This is especially relevant when you or others have referred to the file by using its file name or path.
...
::: {.callout-note collapse="true" icon="true" title="Excursion: Versioning"}
...
- You may use qualifiers such as "raw" or "processed" for data or "draft" or "internal" for documents. However, note that terms such as "final", "final2", "final-revised", "final-changed_again", and "final_ready" can be confusing. In other words: Avoid "final" naming.
...
- Files consist of prefix `amb_hourly_qc_wc4.4_cal6.0_`, followed by year and month (e.g. `2014_08`), followed by `_core-params.csv`. Prefix might be intuitive for researchers of that field, not explicitly described.
- Probably the prefix has some meaning, but it is not explicitly stated in the README.
- Yes, files are sorted according to the month of measurement.
- Yes: File name `AMB hourly, readme.rtf` contains spaces and a comma. The other file names contain several dots (should only be before the file extension `csv`).
...
- Not many files, thus order not important.
...
- Length of the names: OK
- In case folder name `01_09_2022__10_13_33` stands for timestamp 2022-09-01T10:13:33, it could be renamed to `20220901T101333` or `2022-09-01_101333`.
:::
...
::: {.callout-tip collapse="false" icon="true" title="Task 1.3: (\~ 10 minutes)"}
...
::: {.callout-warning collapse="false" icon="true" title="Avoid proprietary formats."}
...
::: {.callout-note collapse="true" icon="true" title="Examples of recommended formats"}
...
- The small files are ASCII or UTF-8 files, thus open. The tar.gz files are compressed TAR-files, also open formats.
...
- The tar.gz files need specific software for extraction, which are freely available, but maybe not installed everywhere, and not all people are familiar with them. Thus it is commandable that the extaction is described in the README file. However, the file size of several GB can be problematic for users having a slow internet connection. And unpacked, the largest file is more than 26 GB, more than the RAM size of many computers.
...
- Most files open: ASCII tables, JSON files, R scripts. But what are "nii.gz" files in folder "results" - maybe zipped [NIfTI](https://en.wikipedia.org/wiki/Neuroimaging_Informatics_Technology_Initiative) files?
...
- ASCII tables an JSON files are easy to understand by humans; nii.gz needs suitable software.
...
- Files are stored as ASCII tables or plain text files, i.e. in open formats.
...
- Format easy to understand by humans, but columns not explicitly described
...
::: {.callout-tip collapse="false" icon="true" title="Task 1.4: (\~ 5 minutes)"}
...
- Special numbers or contents: Some Coluns contain parenthesis - what is their meaning?
