...
As the previous sections have shown, data management is a broad topic encompassing various aspects. Hence, good planning is essential to effectively manage all these aspects according to the [FAIR principles](https://www.go-fair.org/fair-principles/). A valuable tool for this purpose is a (research) data management plan (DMP). This part of the workshop provides an overview of the components of a DMP, offers tips on creating one, and introduces helpful tools. We conclude with a practical exercise where you will work on a small section of a DMP.
...
[![Research data life cycle](\images\RD-Lifecycle.png){width=80%}](https://www.fdm-bayern.org/forschungsdatenmanagement/#fd-lebenszyklus)
...
A data management plan covers all stages of the research data life cycle. Therefore, one can assign questions to each stage. By addressing these questions, you create an effective DMP:
...
- Does your project process personal or sensitive data? What law will be applied? Are there ethnic aspects you need to consider?[^9]
...
Looking at published DMPs, especially from your discipline, may be helpful to gain a deeper insight into the topic. You can find them on different repositories, e.g. [Zenodo](https://zenodo.org/search?q=DMP&l=list&p=1&s=10&sort=bestmatch). 

:::

## Example: Data storage and security

In this section, we approach a DMP from a practical perspective: DMPs are usually divided into logical units, e.g. based on the data life cycle. In the following, we will look at the section "Data storage and security" of a DMP template and provide example answers for two datasets. Before we get started, here is some background information on data storage and security. (If you already feel comfortable with the topic, you can [skip this part](dmp.qmd#questions-in-the-dmp).) 
...
For larger files exceeding 100GB, consider using services such as [GigaMove](https://gigamove.rwth-aachen.de/de) (RWTH Aachen) to share files efficiently. For long-term data preservation with advanced security features, explore [EUDAT B2ACCESS](https://b2access.eudat.eu/home/) – you can find more information about EUDAT services on their [website](https://eudat.eu/catalogue).
...
To protect data against unauthorized access and data misuse is one of the main goals of security.[^7] We recommend using access control mechanisms and encryption to prevent unauthorized access to the data and to protect sensitive information. 
...
The following questions are part of the [RDMO](dmp.qmd#dmp-tool-rdmo) template, section "Data usage / Data storage and security" and "Data usage / Interoperability". For each question, we will provide you with example answers for two datasets. These answers do not rely on any existing project. Also, the data and some tools (e.g. software) are made up. The example answers are based on the information in this [example DMP](https://gitlab.rrze.fau.de/cdi/labs/literacy/proposal-self-service/-/blob/main/RDMO/ERC/ERC_Example.md). The examples are intended to show how specific questions can be answered in a DMP. 
...
::: callout-tip
# Task 4: (~20 min)
If you do not yet have a RDMO account, please register now. Either use the [LMU RDMO](https://rdmo.ub.uni-muenchen.de/) (register using your email or ORCID id) or [MPG RDMO](https://rdmo.mpdl.mpg.de/) (MPG login: Shibboleth). Login and create a new project using the **RDMO template**. 
