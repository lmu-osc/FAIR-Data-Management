...
[^1]: „metadata“. Merriam-Webster Dictionary, accessed 7. July 2024, [www.merriam-webster.com/dictionary/metadata](www.merriam-webster.com/dictionary/metadata).

::: {.callout-tip}
## Task 2.1: (~10 min)

Can you answer all of these questions by looking through your (example) dataset? \
(Concentrate on what you can find on the dataset level and only look further if you have the time.)
...
2.  README file: Before you publish your dataset you might already want to keep track of your metadata. A good way to not loose the overview yourself and inform others is by creating a README file. We will cover README files in the next section.
...
-   Can this dataset be re-used?\
    ...
    + However, the GitHub repository is connected to an Apache License 2.0, which makes it difficult to know what license applies.  
:::
...
-   Where does the provided information of the example README differ from the template below? Mark the sections of the template in \*\*bold\*\* that you can answer solely from the provided README. 
...
Whereas general metadata and README information typically focus on the dataset or project as a whole, codebooks delve deeper, providing detailed explanations and clarifications about the specific data contained in individual files. A code book should make all components of a dataset file self-explanatory. It helps to identify e.g what certain variables and value labels mean or in which format the data points have been collected. This makes the dataset more accessible by allowing re-users to work independent of a research paper. Some people include this information in the README file. However, this can significantly extent the length of the README. It is always best to keep README files concise to maintain their readability.

If all of your data files are structured and labeled according to the same system it is enough to provide one codebook that describes and explain this system. If you have differently structured files you should consider providing multiple codebooks.
...
To improve the quality and usability of your codebook you can provide it as a JSON file. This has the advantage to be machine-readable and -actionable. It makes it easier to extract this information and work with it in coding scripts.
