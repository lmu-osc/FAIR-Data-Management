...
(( CommDW: Sehe ich das richtig, dass an der Stelle noch in keine readme-Datei reingeschaut werden soll, also nur der Inhalt der Datensatz-Seite angeschaut werden soll? Falls ja: "(Concentrate on what you can find on the dataset level, don't look into any README file yet.)" ))
...
-   Can this dataset be re-used?\  (( CommDW: Dieser letzte Punkt wird auch bei der Lizenz bei 'Publication' abgefragt. ))
...
-   Where does the provided information of the example README differ from the template below? Mark the sections of the template in \*\*bold\*\* that you can answer solely from the provided README. (( CommDW: Ich vermute, das Fett-Machen ist nur zur schnelleren Übersicht gedacht? Hilft aber nur, wenn die Teilnehmer gerade einen Markdown-Editor nutzen. Vielleicht einfacher und passender in Hinblick auf den nächsten Punkt: "Count the sections of the template that you can answer solely from the provided README." ))
